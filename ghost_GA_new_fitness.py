...
class GhostChromosome:
    ...
    def compute_fitness(self):
        ...
        current_dist = self.ghost.get_distance_from_pacman()
        times_hit = self.ghost.collisions_with_pacman
        wall_hits = self.ghost.collisions_with_walls
        ghost_hits = self.ghost.collisions_with_ghosts

        """
        # Calculate exploration bonus based on number of unique tiles visited
        num_tiles = self.ghost.maze.num_tiles
        visited_tiles = len(self.ghost.tiles_visited)
        exploration_bonus = (visited_tiles / num_tiles) * 5
        """
        fitness = 1 / (current_dist // TILE_SIZE + 1) + times_hit
        if current_dist < 3 * TILE_SIZE:
            fitness += 1

        # Calculate fitness based on distance, collisions, exploration, and aggression
        previous_dist = self.ghost.distance_from_pacman_prev
        if previous_dist > current_dist:
            fitness += 1
        elif previous_dist < current_dist:
            fitness -= 1
        
        fitness -= wall_hits * 0.5 # Penalty for hitting walls and other ghosts
        fitness -= ghost_hits * 0.5
        fitness += times_hit * 2 # Aggression: ghosts get rewarded for colliding with pacman
